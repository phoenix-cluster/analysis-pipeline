...
def calculate_to_mysql():
    clusters = mysql_acc.get_all_clusters('T_CLUSTER')
    for cluster in clusters:
        scores = conf_sc_calc.calculate_conf_sc_for_a_cluster(cluster)
        cluster['conf_sc'] = json.dumps(scores)

    mysql_acc.upsert_cluster_conf_sc('T_CLUSTER', clusters)


def calculate_to_csv():
    origin_cluster_csv_file = "test_clusters_old.csv"
    new_cluster_csv_file = "test_clusters.csv"
